...
explainer = shap.Explainer(pipeline.named_steps['regressor'], pipeline.named_steps['preprocessor'].transform(X))
...
def get_health_recommendation(user_data):
    ...
    prompt = (
        ...
        f"Predicted calories burned: {predicted_calories:.2f}\n\n"
...
# Function to explain the model's prediction
def explain_prediction(user_data):
    # Transform user data for SHAP
    transformed_data = pipeline.named_steps['preprocessor'].transform(pd.DataFrame([user_data]))

    # Calculate SHAP values
    shap_values = explainer(transformed_data, check_additivity=False)  # Disable additivity check if needed

    # Create a SHAP force plot and return it as HTML
    shap.initjs()  # Initialize JavaScript for SHAP
    shap_plot = shap.force_plot(explainer.expected_value, shap_values.values, transformed_data, matplotlib=False)

    # Return the SHAP plot as HTML
    return shap_plot
...
hear_rate = st.number_input("Heart Rate (bpm)", min_value=30, value=70)
device = st.selectbox("Device", options=["apple watch", "fitbit"])
activity = st.selectbox("Activity Type", options=["Lying", "Sitting", "Self Pace walk", "Running 3 METs", "Running 5 METs", "Running 7 METs"])
...
if st.button("Get Recommendation"):
    ...
    # Get health recommendation and prediction
    recommendation, predicted_calories = get_health_recommendation(user_data)
    
    # Display the recommendation
    st.subheader("Health Recommendation:")
    st.write(recommendation)
    st.write(f"Predicted Calories Burned: {predicted_calories:.2f}")

    # Explain the prediction using SHAP
    st.subheader("Model Explanation:")
    shap_html = explain_prediction(user_data)

    # Display the SHAP plot as HTML

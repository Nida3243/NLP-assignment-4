...
if st.sidebar.button("Get Recommendation"):
    # Prepare user data for prediction ```python
    user_data = {
        'age': age,
    ...
    # Display the recommendation in the specified format
    st.markdown("### Personalized Health Recommendation:")
    st.markdown(f"**Based on the provided user information:**")
    st.markdown(f"**User Data:**")
    st.markdown(f"- Age: **{user_data['age']} years**")
    st.markdown(f"- Gender: **{'Female' if user_data['gender'] == 0 else 'Male'}**")
    st.markdown(f"- Height: **{user_data['height']} cm**")
    st.markdown(f"- Weight: **{user_data['weight']} kg**")
    st.markdown(f"- Steps taken today: **{user_data['steps']}**")
    st.markdown(f"- Activity type: **{user_data['activity']}**")
